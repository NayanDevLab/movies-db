...
export default function MovieDetailsModal() {
  ...
  const { setSelectedActor, setSelectedActorProfile } = useFilters();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/85 backdrop-blur-md overflow-y-auto animate-fadeIn">
      <div className="relative w-full max-w-4xl bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col my-auto max-h-[90vh]">
        {/* Backdrop Banner */}
        <div className="relative h-48 sm:h-64 w-full overflow-hidden bg-slate-950 flex-shrink-0">
          ...
          <div className="absolute top-4 right-4 z-20 flex items-center gap-2">
            <button
              ...
              className={`w-9 h-9 rounded-full flex items-center justify-center border transition-all shadow-lg ${
                isFavorite
                  ...
                  : 'bg-slate-950/80 hover:bg-slate-800 text-slate-300 border-slate-700 hover:text-pink-400'
              }`}
            ...
            <button
              ...
              className="w-9 h-9 rounded-full bg-slate-950/80 hover:bg-slate-800 text-slate-300 hover:text-white flex items-center justify-center border border-slate-700 transition-colors shadow-lg"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="absolute bottom-4 left-6 right-6 flex items-end justify-between">
            <div>
              <span className="px-3 py-1 rounded-full bg-violet-600/60 text-violet-200 border border-violet-500/40 text-xs font-bold uppercase tracking-wider">
            ...
            <div className="hidden sm:flex items-center gap-2">
              <button
                ...
                className="px-3.5 py-1.5 rounded-full bg-slate-900/90 hover:bg-violet-600 text-white border border-slate-700 hover:border-violet-500 font-bold text-xs flex items-center gap-1.5 shadow-lg transition-all"
              >
        ...
        {/* Tab Navigation */}
        <div className="px-6 sm:px-8 pt-3 border-b border-slate-800 bg-slate-950/50 flex items-center gap-4">
          <button
          ...
          >
            <Film className="w-4 h-4 text-violet-400" /> Movie Details & Cast
          </button>
          ...
          >
            <MessageSquare className="w-4 h-4 text-amber-400" /> Audience Reviews (
            {movieReviews.length})
          </button>
        </div>
        ...
        <div className="p-6 sm:p-8 overflow-y-auto space-y-6">
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="hidden md:block">
                  <div className="w-full aspect-[2/3] rounded-2xl overflow-hidden shadow-2xl border border-slate-800">
                    <img
                      src={posterImage}
                      alt={activeMovie.name}
                ...
                <div className="md:col-span-2 space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {activeMovie.genres?.map((g) => (
                      <span
                        key={g}
                        className="px-3 py-1 rounded-lg bg-slate-800 text-slate-200 text-xs font-semibold border border-slate-700/60"
                      >
                        {g}
                      </span>
                    ))}
                  ...
                  {/* Synopsis */}
                  <div>
                    <h3 className="text-xs uppercase tracking-widest text-slate-400 font-bold mb-1 flex items-center gap-1.5">
                      <Film className="w-3.5 h-3.5 text-violet-400" /> Storyline & Overview
                    </h3>
                    <p className="text-slate-300 text-sm leading-relaxed">{cleanSummary}</p>
                  </div>

                  {/* Spec Details Table */}
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 p-3.5 rounded-xl bg-slate-950/60 border border-slate-800/80 text-xs">
                    <div>
              ...
              <div className="pt-4 border-t border-slate-800">
                ...
                {isLoadingCast ? (
                  <div className="py-8 flex items-center justify-center text-xs text-slate-400">
                    <Sparkles className="w-4 h-4 text-violet-400 animate-spin mr-2" />
                    ...
                    {castList.slice(0, 8).map((cast) => {
                      ...
                      return (
                        <div
                          ...
                          className="p-2.5 rounded-xl bg-slate-950/70 border border-slate-800/80 hover:border-violet-500/60 hover:bg-slate-800/50 cursor-pointer transition-all flex items-center gap-3 group"
                        >
                          <div className="w-11 h-11 rounded-full overflow-hidden flex-shrink-0 border border-slate-700 bg-slate-800 group-hover:scale-105 transition-transform">
                            <img
                              src={actorPhoto}
            ...
            <div className="space-y-6">
              ...
              >
                <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-2">
                  <Star className="w-4 h-4 text-amber-400" /> Write an Audience Review
                </h3>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 rounded-xl bg-slate-900 border border-slate-800 text-white placeholder-slate-500 focus:outline-none focus:border-violet-500"
                    />
                  </div>
                ...
                <div>
                  <label className="block text-slate-400 mb-1 text-xs">Your Review</label>
                  <textarea
                    required
                    rows={3}
                    ...
                    className="w-full p-3 rounded-xl bg-slate-900 border border-slate-800 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-violet-500"
                  />
                </div>
                  ...
                  movieReviews.map((rev) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-300 font-bold text-xs">
          ...
          <div className="pt-4 border-t border-slate-800 flex items-center justify-between gap-4">
            <button
              onClick={() => openTrailerModal(activeMovie)}
              className="sm:hidden text-xs text-violet-400 font-bold flex items-center gap-1"
            >
              <Play className="w-3.5 h-3.5 fill-violet-400" /> Trailer
            </button>
            ...
            {activeMovie.officialSite && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-xs text-slate-400 hover:text-white hidden sm:flex items-center gap-1.5 transition-colors"
              >
