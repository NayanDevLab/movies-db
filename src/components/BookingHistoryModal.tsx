      ...
      <div className="relative w-full max-w-2xl bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-950/60">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-white">Your Booked Tickets History</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto space-y-4">
          {bookingHistory.length === 0 ? (
            <div className="py-12 text-center text-slate-400">
              <Ticket className="w-12 h-12 mx-auto mb-3 text-slate-600 opacity-50" />
            ...
            bookingHistory.map((invoice) => (
              <div
                ...
                className="p-4 rounded-xl bg-slate-950/70 border border-slate-800/80 hover:border-violet-500/50 transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-4 group"
              >
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-xs font-bold text-violet-400 px-2 py-0.5 rounded bg-violet-950/50 border border-violet-800/40">
                      {invoice.bookingId}
                    </span>
                ...
                <div className="flex sm:flex-col items-center sm:items-end justify-between gap-2 border-t sm:border-t-0 pt-3 sm:pt-0 border-slate-800">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-violet-600/20 hover:bg-violet-600 text-violet-300 hover:text-white border border-violet-500/30 text-xs font-semibold transition-all"
                  >
